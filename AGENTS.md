...
The frontend uses modular JavaScript objects (`State`, `Storage`, `Theme`, `Settings`, `Toast`, `Terminal_`, `FlashDebug`, `SFTP`, `Hosts`, `UI`) with CSS variables for theming. Color schemes are applied via a `data-color-scheme` attribute on `<html>` (valid values: `default`, `sakura`, `ocean`, `forest`, `twilight`, `amber`) and work orthogonally with the `data-theme` light/dark toggle. The `Settings` module provides an in-app password change form (calls `PUT /api/users/:username/password`) and a color scheme picker; in `ENTRANCE_DESKTOP_NOLOGIN` mode, the password form is hidden and replaced with a notice.
